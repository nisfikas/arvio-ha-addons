...
`arvio.model`, `arvio.batch`, `arvio.trigger_scenario`, `arvio.list_*`, `arvio.*_scenario`, `arvio.pairing_status`, `arvio.zigbee_permit`, `arvio.list_screens`, `arvio.put_screen`, `arvio.delete_screen`, `backup.create`, `agent.update`, και (0.1.21) τα `MEDIA_ACTIONS` της §6.5 (`media_player.*`, `music_assistant.*`, `arvio.media_art|media_browse|media_search`).
...
```bash
docker build -f addons/arvio_agent/Dockerfile.standalone -t arvio-agent:0.1.32 addons/arvio_agent
```
