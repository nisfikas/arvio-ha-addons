...
`arvio.model`, `arvio.batch`, `arvio.trigger_scenario`, `arvio.list_*`, `arvio.*_scenario`, `arvio.pairing_status`, `arvio.zigbee_permit`, `arvio.list_screens`, `arvio.put_screen`, `arvio.delete_screen`, `arvio.device_update`, `arvio.area_update` (`config/area_registry/update`, μόνο relay), `backup.create`, `agent.update`, και (0.1.21) τα `MEDIA_ACTIONS` της §6.5 (`media_player.*`, `music_assistant.*`, `arvio.media_art|media_browse|media_search`).
...
**LAN allowlist** (`LAN_ALLOWED_ACTIONS`, μόνο για τη χωρίς-auth διαδρομή στο :8099 — βλ. §5.4/5): `light.turn_on|turn_off`, `switch.turn_on|turn_off`, `cover.open_cover|close_cover|stop_cover|set_cover_position`, `climate.set_temperature|set_hvac_mode|set_fan_mode|set_preset_mode|set_swing_mode|turn_on|turn_off`, `scene.turn_on`, `script.turn_on` **μόνο** για `script.arvio_*`, τα read-only `arvio.list_entities`, `arvio.pairing_status`, `arvio.list_screens`, το `arvio.trigger_scenario` (σκηνές καταλόγου), και (0.1.21) τα `MEDIA_PLAYER_ACTIONS` + `MUSIC_ASSISTANT_ACTIONS` της §6 (transport, ένταση, ομάδες, ουρά — δεν είναι επικίνδυνα). Τα `arvio.put_screen|delete_screen` είναι **μόνο relay**. Overlay κουδουνιού στο `/panel`: JPEG από `GET /api/screens/doorbell.jpg` και ξεκλείδωμα `POST /api/screens/doorbell/unlock` μόνο για την κάμερα/κλειδαριά που έθεσε ο Partner σε εκείνη την οθόνη — όχι γενικό `lock.unlock` στη LAN διαδρομή. Τα `arvio.media_art|browse|search` (εξώφυλλα, βιβλιοθήκη, playlists = προσωπικά δεδομένα) είναι **μόνο cloud** όσο η LAN διαδρομή είναι χωρίς auth. Είναι allowlist, όχι denylist: ό,τι άλλο υπάρχει στο relay allowlist (lock/alarm, `arvio.model`, `arvio.batch`, `arvio.*` εγγραφές εκτός list/trigger, `backup.*`, `agent.*`, `*.toggle`, κάθε `target`) → `403 lan_forbidden`.
...
5. **LAN** (`POST /v1/hubs/{id}/commands`, `/api/service/{domain}/{service}` χωρίς auth): ρητό **allowlist** (§5.2 «LAN allowlist») — ό,τι δεν είναι εκεί (lock/alarm, `arvio.model`, `arvio.batch`, `arvio.put_screen|delete_screen`, `backup.*`, `agent.*`, `script.turn_on` εκτός `script.arvio_*`, κάθε `target`) → `403 lan_forbidden`· το `payload` δεν προωθείται, άρα το `confirm_dangerous` δεν μπορεί να τεθεί τοπικά (εξαίρεση 0.1.21: για τα `MEDIA_ACTIONS` περνούν **μόνο** τα `LAN_MEDIA_PAYLOAD_KEYS` — ένταση, πηγή, ουρά, query· ποτέ `confirm_dangerous` / `code` / `target`, βλ. §6.6· 0.1.24: `arvio.trigger_scenario` περνά `id|scenario_id`). Ο LAN server απαντά **μόνο same-origin**: κανένα `Access-Control-Allow-Origin` (ούτε στο `OPTIONS`, που επιστρέφει σκέτο `204` + `Allow`), ώστε σελίδα άλλου origin να μην μπορεί να οδηγήσει το hub μέσω browser· τα σερβιριζόμενα `/`, `/home`, `/remote`, `/partner`, `/panel` καλούν σχετικά paths και δουλεύουν όπως πριν. Πάνελ τοίχου: `GET /panel` (χωρίς webfonts), `GET /api/screens` (χωρίς pairing hash), `POST /api/screens/pair`. Τα endpoints κωδικού παρουσίας / claim δεν αλλάζουν εδώ — ξεχωριστή εργασία «LAN hardening: HA ingress».
...
**Wait-for-state:** lights / covers / climate still wait ≤ 2 s. `media_player.*` and `music_assistant.*` do **not** wait — Spotify / Music Assistant hold the HA command socket for seconds, and queuing the next skip/volume behind that wait made Home feel frozen. The hub acks `ok: true` as soon as the service is accepted (`MEDIA_CALL_ASYNC`: the `call_service` runs on a thread; Home already painted the intent). The state stream is the confirmation. `state_snapshot.attrs` of media still carries `platform` from the registry cache.
...
```bash
docker build -f addons/arvio_agent/Dockerfile.standalone -t arvio-agent:0.1.34 addons/arvio_agent
```
